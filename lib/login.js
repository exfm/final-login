...
Login.prototype.onSubmit = function(){
    ...
    for(var i in this.inputs){
        ...
        if(this.validate(val) === false){
            this.triggerError(this.form, i+' cannot be empty');
            return false;
        }
        else{
            data[i] = $.trim(val);
    ...
    if(!this.url){
        this.triggerError(this.form, 'Must provide a url');
        return false;
    }
...
Login.prototype.onResponse = function(jqXhr){
    ...
    if(jqXhr.status === 200){
        this.triggerSuccess(this.form, JSON.parse(jqXhr.responseText)); 
    }
    else{
        this.form.on('submit', $.proxy(this.onSubmit, this));
        this.triggerError(this.form, jqXhr.statusText); 
    }
};
...
// trigger error
Login.prototype.triggerSuccess = function(form, response){
    $(form).trigger(
        'success',
        {
            'response': response
...
// trigger error
Login.prototype.triggerError = function(form, message){
    $(form).trigger(
        'error',
        {
            'message': message
